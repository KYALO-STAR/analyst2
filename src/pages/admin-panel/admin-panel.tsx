...
const AdminPanel: React.FC = () => {
  ...
  const deleteBot = (id: string) =>
    setConfig({ ...config, bots: config.bots.filter(bot => bot.id !== id) });
      ...
      <div className="admin-content">
        ...
        {activeTab === 'channels' && (
          <div className="section">
            ...
            <FormField label="WhatsApp Number" value={config.channels.whatsapp} onChange={(v) => updateChannel('whatsapp', v)} />
            <FormField label="Telegram Handle" value={config.channels.telegram} onChange={(v) => updateChannel('telegram', v)} />
            <FormField label="Email" type="email" value={config.channels.email} onChange={(v) => updateChannel('email', v)} />
            <FormField label="Discord Server" value={config.channels.discord || ''} onChange={(v) => updateChannel('discord', v)} />
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="section">
            ...
            <FormField label="App Name" value={config.appName} onChange={(v) => setConfig({ ...config, appName: v })} />
            <FormField label="Support Email" type="email" value={config.supportEmail} onChange={(v) => setConfig({ ...config, supportEmail: v })} />
          </div>
        )}
      </div>
    </div>
